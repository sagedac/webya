...
**Un solo producto, un solo precio: $100 por página.** Landing de código a medida, siempre construida al mayor nivel de sofisticación visual/interactiva que el proyecto sabe hacer (motor compartido `src/engine/` — `ScrollReveal`, `Parallax`, `ProductVisual` — completo, no una versión recortada) y con los mejores componentes de frontend disponibles para ese negocio (búsqueda en 21st MCP, ver `.claude/agents/template-builder.md`). Toda página comparte la misma base de secciones (navbar, hero, productos/servicios, **por qué elegirnos (3 pilares)**, **cómo pedir (pasos)**, galería, testimonios, **mapa de ubicación (Google Maps embebido)**, horarios/contacto, **formas de pago**, **redes sociales**, **FAQ (acordeón)**, WhatsApp, footer) — lo que varía por negocio es el diseño y el criterio, no cuántas de estas secciones incluye.

**Estado (2026-08-13, fin de los 3 niveles de precio):** decisión explícita de Paul — se retiran los 3 niveles START/PRO/EXPERIENCE como productos con precio distinto (tabla vieja más abajo, dejada como referencia histórica). Ya no tenía sentido cobrar menos por "menos sofisticación" cuando cada página es código a medida de todas formas — no hay plantilla barata de la que amortizar menos trabajo, así que no hay motivo real de costo para ofrecer una versión recortada. El campo `tenants.nivel` sigue existiendo en la base de datos (`tenants_plan_fields_check`, sección 5, todavía `NOT NULL`) pero el panel admin ya no lo pregunta al crear un cliente — se fija en `3` automáticamente (`src/app/admin/(protected)/nuevo/_components/FormularioNuevoTenant.tsx`).
