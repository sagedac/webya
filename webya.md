...
**Estado (2026-08-17, octavo tenant `custom_code`):** `estudio-arquitectura` (`src/custom/estudio-arquitectura/EstudioArquitectura.tsx`) — "Estudio de Arquitectura" (el dueño no dio un nombre comercial más específico; se usa `tenant.nombre` tal cual, mismo patrón del resto de páginas del proyecto, en vez de inventar un nombre/apellido de fundador que no existe), WhatsApp 0939671012. Requisitos funcionales explícitos de Paul: proyectos organizados por categoría (Vivienda y Comercio obligatorias; se agregó "Institucional" como tercera por ser un tipo de encargo real y habitual de un estudio en Ecuador, sin inflar el alcance con una cuarta que no se pidió), cada proyecto con galería de fotos + info (nombre, ubicación, año, área, descripción), y contacto con WhatsApp como CTA principal. Firma visual: "Acotado" — la línea de cota de un plano técnico (`CotaDivider`, `src/custom/estudio-arquitectura/effects.tsx`, se traza con el scroll vía `stroke-dashoffset`, misma técnica que `RouteLine`/`SelesFlow`) y fichas de proyecto con marcas de esquina tipo registro de plano + cuadro de rotulación ("A-01", "A-02"…) en vez de tarjetas de portafolio genéricas — vocabulario visual genuino de la profesión, no un adorno importado de otra página. Patrón de interacción del portafolio (Paul dejó la decisión final al agente constructor): las categorías se filtran con pestañas en línea (no modal, para no anidar modal sobre modal y mantenerlo descubrible); cada proyecto sí abre un modal con galería + info (detalle secundario, no navegación primaria) — razonamiento completo en `src/custom/estudio-arquitectura/ProyectosGaleria.tsx`, adaptando (sin instalar como dependencia) los patrones "Gallery Grid with Lightbox" e "Image Preview" de 21st MCP. No usa `ProductVisual` (una edificación no es un producto físico que "flote"). **Decisión de contenido no trivial de este build:** a diferencia de los defaults de otros tenants (categorías/servicios genéricos, seguros de inventar), los 6 proyectos de muestra que pueblan el portafolio mientras Paul no carga proyectos reales NO llevan nombre propio de cliente ni año — nombrarlos y fecharlos como si fueran obra real entregada habría sido fabricar una historia de portafolio que no existe (webya.md sección 7, "no inventar"), distinto de rellenar una categoría de producto genérica. Cada ficha de muestra lleva además una etiqueta visible "Proyecto de muestra" en la UI (no solo en el código) para que nadie los confunda con obra real; desaparecen automáticamente en cuanto Paul carga `content.precios.categorias` real desde el panel. Mismo criterio aplicado a `content.cifras` (años de experiencia, proyectos completados): sección omitida por completo sin datos reales del admin, sin valor por defecto inventado. Convención de esquema documentada en el propio componente: `cat.items[]` codifica cada proyecto como "Nombre · Ubicación · Año · Área · Descripción" (separador " · ", nunca coma, porque el editor de categorías del panel admin —`EditorContenido.tsx`— une/separa items por coma); las fotos de cada proyecto real se resuelven por convención de nombre de archivo "`{categoría-en-slug}-{índice}-{n}.jpg`" en `content.fotos`, y un proyecto real sin fotos que calcen se muestra sin fotos en vez de tomar prestada la de otro proyecto o una de Unsplash. Sin fotos propias del negocio todavía: 14 fotos de muestra Unsplash (2 por cada uno de los 6 proyectos de muestra, más hero y una foto de "proceso") descargadas a `public/tenants/estudio-arquitectura/`, tracking y atribución en el footer — no debería publicarse como está hasta reemplazarlas por fotos reales de obra propia. Se agregó `"estudio_arquitectura"` (schema.org `Architect`) al enum `Rubro` (`src/lib/types.ts`, `src/lib/json-ld.ts`), mismo criterio que los rubros anteriores. **Verificación de esta sesión:** `tsc --noEmit`, `eslint` y `next build` limpios (sin navegador/Playwright disponible); el tenant real no se dio de alta en Supabase para este build (queda pendiente desde el panel admin, incluida la carga de proyectos y fotos reales).

**Estado (2026-08-17, noveno tenant `custom_code`):** `toyland` (`src/custom/toyland/ToyLand.tsx`) — ToyLand, juguetería de temática anime en el Batán Shopping (Cuenca), 3era planta, WhatsApp 0939671012. Negocio real que ya existía antes del reset del catálogo (era uno de los 8 tenants originales sobre `plantilla_jugueteria`, ver "Registro histórico" abajo, borrado en la limpieza del 2026-08-13 y listado desde entonces en sección 9 como "sin página") — esta build NO es una restauración de aquel código (ya no existe) sino una página nueva de código a medida, que converge en una estética similarmente audaz por ser genuinamente la que le queda bien a este negocio, no por copiar nada. Paleta pedida explícita por Paul: fondo amarillo, con rojo/verde/negro/naranja de acento. `tenant_content.coloresMarca` solo expone 3 colores editables desde el panel admin hoy (`fondo`, `acento`, `texto` — `EditorContenido.tsx` no tiene campo para `secundario1/2/3`), así que el componente usa fondo=amarillo/acento=rojo/texto=negro desde `tenant_content` y dos constantes fijas en código (verde `#2E9E4F`, naranja `#F2811D`) para el resto de la paleta — documentado en el propio componente y pendiente que Paul cargue los 3 colores exactos desde el panel al dar de alta el tenant (si no, hereda el default oscuro/dorado de `emptyContent()`). Firma visual: "En la percha" — cada categoría de producto (Figuras de acción, Funko Pop, Peluches, Accesorios y merch) es una funda blister colgada de un gancho de metal (`GanchoPercha`, SVG) con balanceo idle constante (`HookSway`, nunca se detiene, a diferencia del resto de animaciones de la página que disparan una sola vez al hacer scroll) sobre una pared con textura de perforaciones tipo pegboard — así es una pared de juguetería real, no una grilla de tarjetas genérica — combinada con entradas tipo "pop" de cómic (`PopIn`, rebote exagerado) y una trama de puntos (halftone) como textura de impresión de cómic/manga, ambas en `src/custom/toyland/effects.tsx`. Dirección real (Batán Shopping, 3era planta) usada como fallback de `content.textos.direccion` en el propio componente porque Paul la dio directo en el pedido de este build (no es un placeholder inventado) — a diferencia del resto de tenants, la sección de contacto nunca se omite acá por esa razón; el mapa embebido sigue dependiendo del campo real cargado en el panel (cae a una foto de la tienda si no está). Sin fotos propias del negocio todavía: 6 fotos de muestra Unsplash, elegidas deliberadamente genéricas (máquinas de cápsulas/gachapon, figuras sin franquicia identificable, peluches, llaveros) evitando cualquier foto que muestre un personaje o logo de una franquicia de anime con derechos de autor reconocibles — la licencia del fotógrafo en Unsplash no cubre los derechos del personaje representado, es una capa aparte de riesgo que no vale la pena correr en un mockup. Descargadas a `public/tenants/toyland/`, tracking y atribución en el footer. Se agregó `"jugueteria"` (schema.org `ToyStore`) al enum `Rubro` (`src/lib/types.ts`, `src/lib/json-ld.ts`), mismo criterio que los rubros anteriores. **Verificación de esta sesión:** `tsc --noEmit`, `eslint` y `next build` limpios (sin navegador/Playwright disponible); aplicado de entrada el aprendizaje del bug real de `estudio-arquitectura` (tarjetas de grid sin `w-full` explícito en el wrapper de animación) a las dos grillas repetidas de esta página. El tenant real no se dio de alta en Supabase para este build (queda pendiente desde el panel admin, incluidos los 3 colores de marca exactos, catálogo y fotos reales).

**Estado (2026-08-18, décimo tenant `custom_code` — primer producto propio en vez de negocio físico):** `foodgo` (`src/custom/foodgo/FoodGo.tsx`) — landing de marketplace de delivery de comida para el mercado ecuatoriano (propuesta tipo Rappi/Uber Eats/PedidosYa, pedido explícito de Paul). Distinto de los 9 tenants anteriores: no es un negocio físico existente, es la landing de un producto futuro sin nombre comercial definitivo todavía — "FoodGo" es un placeholder deliberadamente trivial de reemplazar (una sola constante `marca` en el componente, resuelta desde `tenant.nombre` con ese fallback, no texto repetido a mano). Audiencia: comensales que descargan la app, no restaurantes buscando ser aliados (esa sería otra landing, fuera de alcance). Rompe dos reglas de marca de WebYa a propósito, mismo criterio de excepción documentada que usó SAGEDAC: sin WhatsApp como CTA principal (este negocio no tiene canal de WhatsApp — el CTA es "Descargar la app", botones de App Store/Google Play visuales sin backend real, más una barra fija móvil equivalente al botón fijo de WhatsApp del resto del proyecto) y sin dirección/mapa (multi-ciudad, sin una sola ubicación física que mostrar). `content.rubro` usa `"otro"` (mismo valor y mismo motivo que SAGEDAC: no hay un "negocio físico local" que describir con schema.org `LocalBusiness`, ninguno de los rubros existentes calza y no ameritaba agregar uno nuevo para un producto sin alta real). Firma visual: "Ruta en vivo" — una tarjeta de seguimiento de pedido en tiempo real en bucle infinito (stepper de 4 estados, mini-mapa con una ruta punteada que se traza, un repartidor que la recorre, y un ETA que cuenta regresivamente — `LiveTrackingCard`, `src/custom/foodgo/effects.tsx`, construida con SVG + CSS motion path + GSAP, no una captura de pantalla de una app que no existe), reforzada por `RouteDivider` (mismo lenguaje visual de "ruta punteada" conectando los 3 pasos de "Cómo funciona") — es el momento de mayor confianza real de cualquier app de delivery (Rappi/Uber Eats/PedidosYa lo usan igual en su propio marketing), no un adorno importado de otro rubro. No usa `ProductVisual`/`fotoDestacada`: es un marketplace multi-restaurante, no hay un producto físico protagonista. Cifras de la sección de confianza enmarcadas explícitamente como METAS de lanzamiento ("restaurantes aliados", "tiempo de entrega objetivo", "ciudades de lanzamiento"), nunca como historial ya alcanzado — presentar como reales cifras que un producto sin lanzar todavía no tiene habría sido inventar (sección 7); los 4 "restaurantes de muestra" de la sección de marketplace llevan una etiqueta visible "Ejemplo" en la UI, mismo principio que los proyectos de muestra de Estudio de Arquitectura. Paleta pedida explícita por Paul (naranja/rojo apetitoso, cálido y enérgico, sin clonar visualmente a Rappi/Uber Eats): los 3 campos editables de `coloresMarca` se usan para fondo=crema cálido/acento=naranja-rojo/texto=carbón, con rojo profundo/ámbar/carbón oscuro como constantes fijas en código para el resto de la paleta (mismo patrón ya usado por ToyLand) — sin alta real en Supabase, hereda el default oscuro/dorado de `emptyContent()` hasta que se cargue. Fotos: producto sin lanzar, sin restaurantes aliados reales — 9 fotos de muestra Unsplash (comida/delivery genérico) descargadas a `public/tenants/foodgo/`, revisadas una por una para descartar candidatas con cajas de delivery de marcas reales (Telepizza, iFood) o menús de restaurante con nombre propio visible antes de elegir las definitivas, tracking y atribución en el footer. **Verificación de esta sesión:** sin navegador/Playwright disponible en el set de herramientas (a diferencia de la verificación de `travel-agency`) — se armó una ruta de previsualización temporal (`src/app/preview-foodgo-temp/`, con datos simulados y la paleta naranja/rojo real pedida por Paul, borrada antes de terminar) para confirmar contra el dev server ya corriendo que la página renderiza sin errores server-side, que las 9 fotos sirven 200, y que las clases responsive mobile-first esperadas (carrusel de categorías con `snap-x`/`overflow-x-auto`, grillas que colapsan a una columna, barra fija `sm:hidden`) están presentes en el HTML — más una revisión manual del layout a 375px a nivel de código (anchos relativos/`flex-wrap`/`grid-cols-1` en todos lados, sin px fijos) en vez de una captura de pantalla real. `tsc --noEmit`, `eslint` y `next build` limpios. No reemplaza una revisión visual real en un navegador antes de publicar. El tenant real no se dio de alta en Supabase para este build (queda pendiente desde el panel admin, incluido el nombre de marca definitivo, los 3 colores de marca exactos y fotos reales si el negocio llega a tener local/oficina propios).
