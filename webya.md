...
- **Favicon por tenant.** Estado (2026-08-17): `src/app/[slug]/page.tsx` (`generateMetadata`) busca automáticamente `logo.png`/`logo.jpg`/`logo-mark.png`/`favicon.png`/`favicon.ico` dentro de `public/tenants/{slug}/` (mismo orden que ya usa el helper `foto()` de cada componente para el logo del header) y, si encuentra alguno, lo declara como `icons.icon` de esa ruta — así la pestaña del navegador muestra el logo real del negocio en vez del ícono genérico de Next.js. Si el tenant no tiene ninguno de esos archivos todavía, no se declara nada y cae solo al `favicon.ico` global de `src/app/` (el mismo que usan hoy la landing de 26st y el panel admin). No hace falta ningún paso manual nuevo al construir un tenant: si su página ya resuelve un logo real vía `foto()` con uno de esos nombres, el favicon queda resuelto gratis con el mismo archivo.
